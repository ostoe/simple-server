...
func main111thired() {
	...
	flag.StringVar(&listenAddr, "listen", "0.0.0.0:8081", "server listen addr")
	flag.IntVar(&aaaa, "aaaa", 0, "aaaaa.... (default output is 'Hello World')")
	flag.BoolVar(&keepAlive, "keepalive", false, "use HTTP Keep-Alive")
...
func requestHandlerSimple(conn tcpserver.Connection) {
	var leftover [1500]byte

	// rv := reqVarsPool.Get().(*reqVars)

	// bufSize := 0
	// var buf [50]byte
	// conn.SetReadDeadline(nil)
	for {
		_, err := conn.Read(leftover[:])
		if err != nil {
			if err != io.EOF {
